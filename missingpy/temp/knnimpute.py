    ...
    * Olga Troyanskaya, Michael Cantor, Gavin Sherlock, Pat Brown, Trevor
    ...
    def _impute(self, dist, X_imp, fitted_X, mask, mask_fx):
        ...
        for c in range(n_cols_X):
            ...
            # Final imputation
            if type(fitted_X_temp.iloc[0,c]) is str:
                s = set(donors.flatten())
    ...
    def transform(self, X):
        ...
        if np.any(row_has_missing):
            ...
            for col in self.fitted_X_.columns:
                if type(self.fitted_X_.loc[~mask_fx[col], col].iloc[0]) is str:
                    l.append(list(set(self.fitted_X_.loc[~mask_fx[col], col])))
                else:
            ...
            dist[row_has_missing] = pairwise_distances(
                ...
                missing_values=self.missing_values, l=l)

            # Find and impute missing
            X = self._impute(dist, X, self.fitted_X_, mask, mask_fx)
            X_trans = pd.DataFrame()
            for i,col in enumerate(self.fitted_X_.columns):
                if type(self.fitted_X_.loc[~mask_fx[col], col].iloc[0]) is str:
                    for c in l[i]:
                        X_trans[col+"_"+c] = (X.iloc[:,i]==c)*1
                else:
                    X_trans[col] = X.iloc[:,i]
